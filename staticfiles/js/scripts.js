...
---------------------------------------------------*/	
	...
	//play embedded content vimeo videos
	$('.vimeo-embed').click(function(){
		var vimeo_url = $('.embedded-video-iframe', this).attr('src');
		var vimeo_url_play = vimeo_url+'&autoplay=1';
		$('.embedded-video-iframe', this).attr('src', vimeo_url_play);
		$('.embedded-video', this).fadeIn(600);
	});


	//close embedded content vimeo videos
	$('.vimeo-embed .close-video').click(function(e){
		var video_wrapper = $(this).parent('.embedded-video');
		$(video_wrapper).fadeOut(600, function(){
			var vimeo_id = video_wrapper.data('vimeo-id');
			var vimeo_url = 'https://player.vimeo.com/video/'+vimeo_id+'?&color=ffffff&title=0&byline=0&portrait=0';
			$('.embedded-video-iframe', video_wrapper).attr('src', vimeo_url);
		});
		e.stopPropagation();
	});
